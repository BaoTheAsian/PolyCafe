...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .time-chip { font-size:12px; color:rgba(44,24,16,.5); background:var(--foam);
                 border:1px solid var(--steam); padding:6px 14px; border-radius:20px;
                 display:flex; align-items:center; gap:6px; }
    .notif-btn { width:36px; height:36px; border-radius:50%; background:var(--foam);
                 border:1px solid var(--steam); display:flex; align-items:center;
                 justify-content:center; cursor:pointer; position:relative; text-decoration:none; }
    .notif-dot { position:absolute; top:7px; right:7px; width:7px; height:7px;
                 border-radius:50%; background:var(--warm-red); border:1.5px solid var(--cream); }

    /* ── Admin stat cards ── */
    .stats-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:26px; }
    .stat-card { background:#fff; border:1px solid var(--steam); border-radius:14px; padding:20px;
                 position:relative; overflow:hidden; cursor:pointer; transition:transform .2s, box-shadow .2s; }
    ...
    .stat-label { font-size:11px; color:rgba(44,24,16,.45); text-transform:uppercase;
    ...
    .card-header { padding:18px 22px 14px; border-bottom:1px solid var(--steam);
                   display:flex; align-items:center; justify-content:space-between; }
    .card-title  { font-family:'Playfair Display',serif; font-size:15px; font-weight:500; }
    .card-action { font-size:12px; color:var(--latte); text-decoration:none; font-weight:500; }
    .card-action:hover { color:var(--roast); }
    .card-body   { padding:20px 22px; }
    .chart-area  { height:160px; display:flex; align-items:flex-end; gap:6px; }
    .order-item  { display:flex; align-items:center; gap:12px; padding:11px 0;
                   border-bottom:1px solid var(--steam); text-decoration:none; transition:background .12s; }
    ...
    /* ── Quick links ── */
    .divider-label { font-size:11px; color:rgba(44,24,16,.35); letter-spacing:1.5px;
    ...
    .quick-link { background:#fff; border:1px solid var(--steam); border-radius:14px;
    ...
    /* ── POS CTA ── */
    .pos-cta { background:var(--espresso); border-radius:14px; padding:22px 26px;
               display:flex; align-items:center; gap:16px; text-decoration:none;
               transition:background .2s; position:relative; overflow:hidden; }
    .pos-cta::before { content:''; position:absolute; right:-30px; top:-30px;
                       width:120px; height:120px; border-radius:50%; background:rgba(200,149,108,.1); }
    .pos-cta:hover { background:var(--mocha); }
    .pos-icon { width:48px; height:48px; border-radius:14px; background:rgba(200,149,108,.15);
    ...
    .pos-btn { margin-left:auto; background:var(--latte); color:#fff; border:none;
    ...
    .emp-welcome { background:#fff; border:1px solid var(--steam); border-radius:14px;
                   padding:32px; text-align:center; margin-bottom:20px; }
    .emp-welcome-title { font-family:'Playfair Display',serif; font-size:22px; margin-bottom:8px; }
    .emp-welcome-sub { font-size:14px; color:rgba(44,24,16,.5); margin-bottom:24px; }
    .emp-role-badge { display:inline-flex; align-items:center; gap:6px; padding:4px 14px;
                      border-radius:20px; background:rgba(200,149,108,.12);
                      border:1px solid rgba(200,149,108,.25); color:var(--roast);
...
<main class="main">
  ...
  <div class="topbar">
    <div>
      ...
      <div class="page-sub">
        <c:choose>
          <c:when test="${sessionScope.user.role eq 'manager' or sessionScope.user.role eq 'true' or sessionScope.user.role eq '1'}">
            Tổng quan hoạt động hôm nay của PolyCoffee
          </c:when>
          <c:otherwise>Chúc bạn làm việc vui vẻ hôm nay</c:otherwise>
        </c:choose>
      </div>
    </div>
    <div class="topbar-right">
      ...
      <c:if test="${sessionScope.user.role eq 'manager' or sessionScope.user.role eq 'true' or sessionScope.user.role eq '1'}">
  ...
  <c:when test="${sessionScope.user.role eq 'manager' or sessionScope.user.role eq 'true' or sessionScope.user.role eq '1'}">
    ...
    <div class="stats-grid">
      <div class="stat-card revenue" onclick="location.href='${pageContext.request.contextPath}/manager/statistics'">
        <div class="stat-icon"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="#C9943A" stroke-width="1.5"><circle cx="9" cy="9" r="7.5"/><path d="M9 5.5v7M7 11.5c0 .83.67 1.5 2 1.5s2-.67 2-1.5-1-1.5-2-1.5-2-.67-2-1.5S8.17 8 9 8s2 .67 2 1.5"/></svg></div>
      ...
      <div class="stat-card orders" onclick="location.href='${pageContext.request.contextPath}/manager/bills'">
        <div class="stat-icon"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="#C8956C" stroke-width="1.5"><path d="M2.5 4.5h13L14 14H4L2.5 4.5z"/><path d="M6.5 4.5V3a2 2 0 014 0v1.5"/></svg></div>
      ...
      <div class="stat-card staff" onclick="location.href='${pageContext.request.contextPath}/manager/staffs'">
        <div class="stat-icon"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="#3D7A5C" stroke-width="1.5"><circle cx="9" cy="6" r="3"/><path d="M3.5 16c0-3 2.5-5 5.5-5s5.5 2 5.5 5"/></svg></div>
      ...
      <div class="stat-card pending" onclick="location.href='${pageContext.request.contextPath}/manager/bills'">
        <div class="stat-icon"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="#C04B3E" stroke-width="1.5"><circle cx="9" cy="9" r="7.5"/><path d="M9 5.5v4l2.5 2.5"/></svg></div>
          ...
          <c:forEach var="b" items="${recentBills}">
            <a class="order-item" href="${pageContext.request.contextPath}/manager/bills?action=detail&id=${b.id}">
              ...
              <div class="order-info">
                ...
                <div class="order-meta">${staffNames[b.userId]} · <fmt:formatDate value="${b.createdAt}" pattern="HH:mm"/></div>
              </div>
              <div class="order-amount"><fmt:formatNumber value="${b.total}" type="number" groupingUsed="true" maxFractionDigits="0"/>đ</div>
    ...
    <div style="margin-bottom:12px;">
      <a class="pos-cta" href="${pageContext.request.contextPath}/employee/pos">
        <div class="pos-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C8956C" stroke-width="1.6"><rect x="2" y="4" width="20" height="14" rx="2"/><path d="M8 18v2M16 18v2M5 18h14"/><path d="M9 9h6M12 7v4"/></svg></div>
        <div>
  ...
  <%-- ═══════════════════════════════════════════
       EMPLOYEE VIEW — placeholder until populated
  ═══════════════════════════════════════════ --%>
  <c:otherwise>

    <div class="emp-welcome">
      <div style="font-size:48px;margin-bottom:12px;">☕</div>
      <div class="emp-welcome-title">Chào mừng, ${sessionScope.user.fullName}!</div>
      <div class="emp-welcome-sub">Bạn đã đăng nhập thành công vào hệ thống PolyCoffee.</div>
      <div class="emp-role-badge">
        <c:choose>
          <c:when test="${sessionScope.user.role eq 'cashier'}">Thu ngân</c:when>
          <c:when test="${sessionScope.user.role eq 'barista'}">Pha chế</c:when>
          <c:otherwise>Nhân viên</c:otherwise>
        </c:choose>
      </div>
      <div>
        <a href="${pageContext.request.contextPath}/employee/pos" class="pos-cta"
           style="max-width:500px;margin:0 auto;display:flex;">
          <div class="pos-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C8956C" stroke-width="1.6"><rect x="2" y="4" width="20" height="14" rx="2"/><path d="M8 18v2M16 18v2M5 18h14"/><path d="M9 9h6M12 7v4"/></svg></div>
          <div>
            <div class="pos-title">Bắt đầu bán hàng</div>
            <div class="pos-desc">Mở màn hình POS để tạo đơn</div>
          </div>
          <button class="pos-btn">Mở POS →</button>
        </a>
      </div>
...
<script>
  ...
  // Revenue bar chart — only runs for manager (chartRevenue defined via JSP)
  (function() {
